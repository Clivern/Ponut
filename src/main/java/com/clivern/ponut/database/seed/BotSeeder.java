...
public class BotSeeder extends Seeder {
    ...
    public void up()
    {
        ...
        this.seeds.put("02-up_insert1_into_bots_table", String.format("INSERT INTO %s " +
            "(`name`, `slug`, `status`, `type`, `isDefault`, `created`, `updated`) VALUES " +
            "('Messenger', 'messenger', 'running', 'messenger_bot', 'no', '" + df.format(todaysDate) + "', '" + df.format(todaysDate) + "');",
        this.tableName));
        this.seeds.put("03-up_insert2_into_bots_table", String.format("INSERT INTO %s " +
            "(`name`, `slug`, `status`, `type`, `isDefault`, `created`, `updated`) VALUES " +
            "('Slack', 'slack', 'running', 'slack_bot', 'no', '" + df.format(todaysDate) + "', '" + df.format(todaysDate) + "');",
        this.tableName));
    }
